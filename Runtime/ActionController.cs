...
#region Effect Management
        ...
        public bool UnregisterEffect(string actionName, EffectBase effect)
        {
            ...
            if (removed)
            {
                ...
                Debug.Log($"[ActionController] Effect '{effect.name}' unregistered from Action '{actionName}'");
            }
            else
            {
                Debug.LogWarning($"[ActionController] Effect '{effect.name}' not found in Action '{actionName}'");
            }
        ...
        public bool UnregisterEffect(int actionHash, EffectBase effect)
        {
            ...
            if (removed)
            {
                ...
                effect.Dispose();
                this.onEffectUnregistered?.Invoke(action, effect);
                Debug.Log($"[ActionController] Effect '{effect.name}' unregistered from Action '{action.actionName}'");
            }
            else
            {
                Debug.LogWarning($"[ActionController] Effect '{effect.name}' not found in Action '{action.actionName}'");
            }
